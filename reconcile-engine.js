...
window.ReconcileEngine = (() => {
  ...
  function createAccumulator() {
    return { qtyMap:new Map(), sourceMap:new Map(), errors:[], rowsRead:0, sheetsRead:0 };
  }

  function processEntry(entry, acc) {
    if (!entry || !entry.workbook) return acc;
    ...
    for (const sheetName of workbook.SheetNames) {
      ...
      if (!headers) {
        acc.errors.push(`${entry.file.name} / ${sheetName}: kolom productSapCode atau qtyFix tidak ditemukan.`);
        continue;
      }
      ...
      for (let rowIndex=headers.headerRow+1; rowIndex<rows.length; rowIndex++) {
        ...
        acc.qtyMap.set(sap,(acc.qtyMap.get(sap)||0)+qty);
        if (!acc.sourceMap.has(sap)) acc.sourceMap.set(sap,[]);
        acc.sourceMap.get(sap).push({file:entry.file.name,sheet:sheetName,row:rowIndex+1,qty});
        acc.rowsRead++;
      }
    }
    return acc;
  ...
  function processMultiple(soEntries) {
    ...
    if (!Array.isArray(soEntries)) {
      acc.errors.push("Data file SO tidak valid.");
      return acc;
    }
    for (const entry of soEntries) processEntry(entry,acc);
    return acc;
  }
